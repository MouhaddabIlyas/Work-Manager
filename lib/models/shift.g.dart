// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'shift.dart';

// **************************************************************************
// TypeAdapterGenerator
// **************************************************************************

class ShiftAdapter extends TypeAdapter<Shift> {
  @override
  final int typeId = 1;

  @override
  Shift read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return Shift(
      date: fields[0] as String,
      shiftStart: fields[1] as String,
      shiftEnd: fields[2] as String,
      workedHours: fields[3] as String,
      workHours: fields[4] as String,
      breakDuration: fields[5] as String,
      money: fields[6] as double,
      meal: fields[7] as bool,
      shiftWage: fields[8] as int,
      note: fields[9] as String,
      isVacation: fields[10] as bool,
    );
  }

  @override
  void write(BinaryWriter writer, Shift obj) {
    writer
      ..writeByte(11)
      ..writeByte(0)
      ..write(obj.date)
      ..writeByte(1)
      ..write(obj.shiftStart)
      ..writeByte(2)
      ..write(obj.shiftEnd)
      ..writeByte(3)
      ..write(obj.workedHours)
      ..writeByte(4)
      ..write(obj.workHours)
      ..writeByte(5)
      ..write(obj.breakDuration)
      ..writeByte(6)
      ..write(obj.money)
      ..writeByte(7)
      ..write(obj.meal)
      ..writeByte(8)
      ..write(obj.shiftWage)
      ..writeByte(9)
      ..write(obj.note)
      ..writeByte(10)
      ..write(obj.isVacation);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ShiftAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}
